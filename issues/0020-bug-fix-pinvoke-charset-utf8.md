...
- Priority: Critical
- Created: 2026-08-27
- Branch: feature/fix-pinvoke-charset-utf8
- Polished: 2026-08-30
- Milestone: 2026.2.0
...
```csharp
[DllImport(DllName)]
static extern void sora_send_message(IntPtr p, string label, [In] byte[] buf, int size);
```

同ファイル内の各種コールバック delegate 定義 (例: `AddTrackCallbackDelegate` / `MessageCallbackDelegate` / `RpcCallbackDelegate`) にも `[UnmanagedFunctionPointer(CallingConvention.Cdecl, CharSet = CharSet.Ansi)]` のような CharSet 指定がない:
...
- 全 `[DllImport(DllName)]` に `CharSet` を明示指定する
  ...
  - なお `[LibraryImport]` (.NET 7 の source-generated P/Invoke) は、Unity の Roslyn (C# 9) と BCL に `LibraryImportAttribute` が無いため利用できない。対象外とする
- 各 `[DllImport]` の `string` 引数は (a) を主方針として対応する
  - (a) `[DllImport(DllName, CharSet = CharSet.Ansi)]` にした上で、各 `string` 引数に `[MarshalAs(UnmanagedType.LPUTF8Str)]` を付与する
  - Unity 公式ドキュメント (plug-ins-native-pass-strings) が推奨する方法であり、`LPUTF8Str` は正しい UTF-8 バイト列をネイティブ側へ渡す
  - (b) `string` を `byte[]` (UTF-8 バイト列) に変更し、C# 側で `Encoding.UTF8` を使って手動マーシャリングする。何らかのバックエンドで (a) が機能しないと判明した場合のみ採用する
- delegate 定義にも `[UnmanagedFunctionPointer(CallingConvention.Cdecl, CharSet = CharSet.Ansi)]` を付与し、各 `string` 引数に `[MarshalAs(UnmanagedType.LPUTF8Str)]` を付ける
- Sora.cs 内で string を扱う全境界を体系的に洗い出し、抜け漏れが無いことを保証する

## 完了条件

- `SoraUnitySdkExamples/Assets/SoraUnitySdk/Sora.cs` の全 `[DllImport]` と全 delegate 定義のうち、`string` を扱う境界に UTF-8 マーシャリングが施されている
