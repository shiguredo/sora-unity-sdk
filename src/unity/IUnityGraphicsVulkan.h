...
struct UnityVulkanInstance {
  VkPipelineCache
      pipelineCache;  // Unity's pipeline cache is serialized to disk
  VkInstance instance;
  VkPhysicalDevice physicalDevice;
  VkDevice device;
  VkQueue graphicsQueue;
  PFN_vkGetInstanceProcAddr
      getInstanceProcAddr;  // vkGetInstanceProcAddr of the Vulkan loader, same as the one passed to UnityVulkanInitCallback
...
struct UnityVulkanMemory {
  ...
  VkDeviceSize
      size;  // size in bytes, may be less than the total size of memory;
  void*
      mapped;  // pointer to mapped memory block, NULL if not mappable, offset is already applied, remaining block still has at least the given size.
  VkMemoryPropertyFlags flags;  // Vulkan memory properties
  unsigned int
      memoryTypeIndex;  // index into VkPhysicalDeviceMemoryProperties::memoryTypes
...
struct UnityVulkanBuffer {
  ...
  size_t
      sizeInBytes;  // size of the buffer in bytes, may be less than memory size
...
struct UnityVulkanRecordingState {
  VkCommandBuffer
      commandBuffer;  // Vulkan command buffer that is currently recorded by Unity
  VkCommandBufferLevel commandBufferLevel;
  VkRenderPass
      renderPass;  // Current render pass, a compatible one or VK_NULL_HANDLE
  ...
  unsigned long long
      currentFrameNumber;  // can be used to track lifetime of own resources
  unsigned long long
      safeFrameNumber;  // all resources that were used in this frame (or before) are safe to be released
...
enum UnityVulkanEventConfigFlagBits {
  ...
  kUnityVulkanEventConfigFlag_FlushCommandBuffers =
      (1 << 1),  // submit existing command buffers, default: not set
  kUnityVulkanEventConfigFlag_SyncWorkerThreads =
      (1 << 2),  // wait for worker threads to finish, default: not set
  kUnityVulkanEventConfigFlag_ModifiesCommandBuffersState =
      (1
       << 3),  // should be set when descriptor set bindings, vertex buffer bindings, etc are changed (default: set)
};
...
typedef PFN_vkGetInstanceProcAddr(UNITY_INTERFACE_API* UnityVulkanInitCallback)(
...
UNITY_DECLARE_INTERFACE(IUnityGraphicsVulkan) {
  ...
  bool(UNITY_INTERFACE_API * InterceptInitialization)(
      UnityVulkanInitCallback func, void* userdata);
  ...
  PFN_vkVoidFunction(UNITY_INTERFACE_API * InterceptVulkanAPI)(
      const char* name, PFN_vkVoidFunction func);

  // Change the precondition for a specific user-defined event
  // Should be called during initialization
  void(UNITY_INTERFACE_API * ConfigureEvent)(
      int eventID, const UnityVulkanPluginEventConfig* pluginEventConfig);
  ...
  bool(UNITY_INTERFACE_API * CommandRecordingState)(
      UnityVulkanRecordingState * outCommandRecordingState,
  ...
  bool(UNITY_INTERFACE_API * AccessTexture)(
      void* nativeTexture, const VkImageSubresource* subResource,
      VkImageLayout layout, VkPipelineStageFlags pipelineStageFlags,
      VkAccessFlags accessFlags, UnityVulkanResourceAccessMode accessMode,
      UnityVulkanImage* outImage);

  bool(UNITY_INTERFACE_API * AccessRenderBufferTexture)(
      UnityRenderBuffer nativeRenderBuffer,
      const VkImageSubresource* subResource, VkImageLayout layout,
      VkPipelineStageFlags pipelineStageFlags, VkAccessFlags accessFlags,
      UnityVulkanResourceAccessMode accessMode, UnityVulkanImage* outImage);
  ...
  bool(UNITY_INTERFACE_API * AccessBuffer)(
      void* nativeBuffer, VkPipelineStageFlags pipelineStageFlags,
      VkAccessFlags accessFlags, UnityVulkanResourceAccessMode accessMode,
  ...
  void(UNITY_INTERFACE_API * AccessQueue)(
      UnityRenderingEventAndData, int eventId, void* userData, bool flush);

  // Configure swapchains that are created by Unity.
  // Must be called before kUnityGfxDeviceEventInitialize (preload plugin)
  bool(UNITY_INTERFACE_API * ConfigureSwapchain)(
      const UnityVulkanSwapchainConfiguration* swapChainConfig);

  // see AccessTexture
  // Accepts UnityTextureID (UnityRenderingExtTextureUpdateParamsV2::textureID, UnityRenderingExtCustomBlitParams::source)
  bool(UNITY_INTERFACE_API * AccessTextureByID)(
      UnityTextureID textureID, const VkImageSubresource* subResource,
      VkImageLayout layout, VkPipelineStageFlags pipelineStageFlags,
      VkAccessFlags accessFlags, UnityVulkanResourceAccessMode accessMode,
      UnityVulkanImage* outImage);
};
UNITY_REGISTER_INTERFACE_GUID(0x95355348d4ef4e11ULL,
